...
def task(args):
    ...
    bin = bin.joinpath("freezer-rs")

    bin_module = temp_dir.joinpath("freezer-rs")
    shutil.copy2(bin, bin_module)
    tools.strip(bin_module)

    build_time = datetime.now().strftime("%Y-%m-%d-%Hh%Mm%Ss")
    build_type = "release" if release else "debug"
    output = Path("output") / f"freezer-rs_{build_type}_{build_time}"
    ...
    print("freezer-rs build successfully: {}.zip".format(output))
